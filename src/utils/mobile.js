    ...
    if (os === `ios`) {
        ...
        `resize orientationchange`.split(` `).forEach(type =>
            window.addEventListener(type, () => {
                onKeyboard(window.innerHeight < height); //TODO on resize desktop browser
            })
        );
    }
});
...
function onKeyboard(isOpen) {
    if (isOpen) {
        ...
        document.body.children[2].children[0].innerHTML = `keyboard on: ${viewport.getAttribute(`content`)}`;
    } else {
        document.body.children[2].children[0].innerHTML = `keyboard off`;
